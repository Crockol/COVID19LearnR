...
data <-  read_excel("DataECDC/COVID-19-geographic-disbtribution-worldwide-2020-03-25.xlsx")
```
...
```{r}
colnames(data) <- c("DateRep","Day" ,"Month","Year","Cases","Deaths", "Countries","GeoId", "Pop_Data.2018")
colnames(data)
...
```{r, warning=FALSE,fig.width=8, fig.height=7} 
Italy$pred <- exp(predict(lm(log(Cases+0.01) ~ DateRep , data = Italy)))
ggplot(Italy, aes(x =DateRep, y = Cases)) +
  geom_point() + 
  geom_line(aes(y = pred)) +
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
...
Germany$pred <- exp(predict(lm(log(Deaths+0.01)~ DateRep , data = Germany)))
ggplot(Germany, aes(x =DateRep, y = Deaths)) +
  geom_point() + 
  geom_line(aes(y = pred)) +
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
