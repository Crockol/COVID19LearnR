...
library(plotly)
library(htmlwidgets)
data <-  read_excel("DataECDC/COVID-19-geographic-disbtribution-worldwide-2020-03-17.xlsx")
```
...
Here I transform the names of the column to avoid spaces. This is a classical mistake that is better to avoid when preparing your database. Then proceed with exploring some of the data inside the dataframe.

```{r}
colnames(data) <- c("DateRep","Day" ,"Month","Year","Cases","Deaths", "Countries","GeoId")
colnames(data)
...
data <-  read_excel("DataECDC/COVID-19-geographic-disbtribution-worldwide-2020-03-17.xlsx")
colnames(data) <- c("DateRep","Day" ,"Month","Year","Cases","Deaths", "Countries","GeoId")
col <- topo.colors(length(unique(data$Countries)))
names(col) <- unique(data$Countries)
...
ggplot(March14, aes(Countries, Cases, col= col[March14$Countries],)) +
  geom_col()+
...
March14 <- data[which(data$Date=="2020-03-14"),]
ggplot(March14, aes(Countries, Deaths, col= col[March14$Countries],)) +
  geom_col()+
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
  theme(legend.position="none")
```
...
Italy <- data[which(data$Countries=="Italy"),]
  p <- ggplot(Italy, aes(x= DateRep, y= Cases, col= col["Italy"])) + 
    geom_point() + 
    ggtitle("Italy") +
    theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
    theme(legend.position = "none") 
...
China <- data[which(data$Countries=="China"),]
  p <- ggplot(China, aes(x= DateRep, y= Cases, col= col["China"])) + 
    geom_point() + 
    ggtitle("China") +
    theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
    theme(legend.position = "none") 
...
Germany <- data[which(data$Countries=="Germany"),]
  p <- ggplot(Germany, aes(x= DateRep, y= Cases, col= col["Germany"])) + 
    geom_point() + 
    ggtitle("Germany") +
    theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
    theme(legend.position = "none") 
...
```{r, fig.height=6, fig.width=8}
for (i in unique(data$Countries)[1:10]) {
  tmp <- data[which(data$Countries==i),]
  p <- ggplot(tmp, aes(x= DateRep, y= Cases, col= col[i])) + 
    geom_point() + 
    ggtitle(i) +
    theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
    theme(legend.position = "none")  
...
for (i in unique(data$EU)) {
  ...
  p <- ggplot(tmp, aes(x= DateRep, y= Deaths)) + 
    geom_point() + 
    ggtitle(i) +
    theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
    theme(legend.position = "none") 
...
Italy$pred <- predict(lm(Cases ~ DateRep , data = Italy))
ggplot(Italy, aes(x =DateRep, y = Cases)) +
  geom_point() + 
...
```{r, warning=FALSE,fig.width=8, fig.height=7} 
Italy$pred <- exp(predict(lm(log2(Cases+0.01) ~ DateRep , data = Italy)))
ggplot(Italy, aes(x =DateRep, y = Cases)) +
  geom_point() + 
  geom_line(aes(y = pred)) +
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
...
Germany <- data[which(data$Countries=="Germany"),]
Germany$pred <- predict(lm(Deaths ~ DateRep , data = Germany))
ggplot(Germany, aes(x =DateRep, y = Deaths)) +
  geom_point() + 
  geom_line(aes(y = pred)) +
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
  theme(legend.position = "none") 
  
Germany$pred <- exp(predict(lm(log10(Deaths+0.01)~ DateRep , data = Germany)))
ggplot(Germany, aes(x =DateRep, y = Deaths)) +
  geom_point() + 
  geom_line(aes(y = pred)) +
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
...
ggplot(data, aes(Countries, totalCases, col=col[data$Countries])) +
  geom_col()+
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
  theme(legend.position="none")
...
ggplot(data, aes(Countries, log(totalCases+0.1), col=col[data$Countries])) +
  geom_col()+
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
  theme(legend.position="none")
...
for (i in unique(data$Countries)) {
  ...
  data$totalCasesPerDay[which(data$Countries==i)] <- tmp$totalCasesPerDay
}

scatterPlot <- ggplot(data, aes(x =DateRep, y = totalCasesPerDay, text =Countries)) +
  geom_point(colour = col[data$Countries]) + 
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1)) +
  theme_classic()
...
ggplot(data, aes(Countries, totalDeaths, col=col[data$Countries])) +
  geom_col()+
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1))+
  theme(legend.position="none")
```
...
scatterPlot <- ggplot(data, aes(x =DateRep, y = totalDeathsPerDay, text =Countries)) +
  geom_point(colour = col[data$Countries]) + 
  theme(axis.text.x= element_text(angle = 90, hjust = 1, vjust = 1)) +
  theme_classic()
